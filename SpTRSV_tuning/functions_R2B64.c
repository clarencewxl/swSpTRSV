/*************************************************************************
	> File Name: functions_R2B64.c
	> Author: Wang Xinliang
	> mail: clarencewxl@gmail.com
	> Created Time: Mon 13 Feb 2017 06:35:07 PM CST
 ************************************************************************/

#define PRODUCER_CONSUMER_ROWS      2
#define PRODUCER_CONSUMER_ROWS_LOG2 1
#define CACHE_X_V4                  64
#define CACHE_X_V4_LOG2             6
...
extern SLAVE_FUN(General_Solver_CSC_Multiple_MPE_R2B64)(void*);
...
int Parallel_Solver_Compress_General_R2B64(double *x, Matrix_Compress_General *m, double *b, int times)
{
	...
	m->b = b;
    m->times = times;
    athread_spawn(General_Solver_CSC_Multiple_MPE_R2B64, m);
	athread_join();
	return 0;
}
